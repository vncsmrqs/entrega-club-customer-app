...
<template>
  <MainViewLayout class="flex flex-col">
    ...
    <div class="content flex-1 overflow-y-auto p-4">
      <template v-if="customerAddressStore.loading">
        <div
          class="w-full h-full text-red-500 flex justify-center items-center"
        >
          <LoaderComponent />
        </div>
      </template>
      <template v-else-if="customerAddressStore.error">
        <div
        ...
        <div class="flex flex-col gap-4">
          <Transition name="fade" mode="out-in" appear :duration="500">
            <div v-if="['SEARCH_ADDRESS', 'ADDRESS_SELECTION'].includes(step)">
              <div class="sticky top-0 lef-0 w-full bg-white">
